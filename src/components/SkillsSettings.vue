...
<template>
  <div class="max-w-4xl mx-auto px-4 py-8">
    ...
    <div class="flex items-start justify-between gap-3 mb-6">
      <div>
        <h3 class="text-base font-semibold text-gray-700 dark:text-gray-200 mb-1">Skills 技能</h3>
    ...
    <div
      v-if="error"
      class="rounded-2xl border border-red-200 dark:border-red-900/50 bg-red-50 dark:bg-red-900/20 px-4 py-3 flex items-center gap-2 text-sm text-red-600 dark:text-red-400 mb-6"
    >
      <AlertCircle :size="16" />
    ...
    <!-- 加载态 -->
    <div v-if="loading" class="py-20 flex flex-col items-center gap-3 text-gray-400 dark:text-gray-500">
      <a-spin />
    ...
    <div
      ...
      class="rounded-2xl border border-dashed border-gray-300 dark:border-[var(--color-border)] bg-white/50 dark:bg-[var(--color-bg-subtle)]/50 px-4 py-16 text-center"
    >
      <Sparkles :size="28" class="mx-auto text-gray-300 dark:text-gray-600 mb-3" />
      <p class="text-sm font-medium text-gray-600 dark:text-gray-300 mb-1">未发现技能</p>
      <p class="text-xs text-gray-400 dark:text-gray-500 leading-relaxed">
        可点击右上角「从其他 Agent 导入」复制本机其他 Agent 的技能，<br />
        或在每个技能目录下放置 <code class="text-gray-500 dark:text-gray-400 bg-gray-100 dark:bg-[var(--color-bg)] px-1 rounded">SKILL.md</code><br />
        后端会自动扫描项目根 <code class="text-gray-500 dark:text-gray-400 bg-gray-100 dark:bg-[var(--color-bg)] px-1 rounded">skills/</code> 及用户主目录下的约定位置
      </p>
    </div>
    ...
    <div v-else class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
      <div
        v-for="s in skills"
        :key="s.id"
        class="rounded-2xl border border-gray-200 dark:border-[var(--color-border)] bg-white dark:bg-[var(--color-bg-subtle)] shadow-sm p-4 transition-all duration-150 hover:-translate-y-0.5 hover:shadow-md"
      ...
      >
        <div class="flex items-start justify-between gap-2">
          <div class="flex items-center gap-2.5 min-w-0">
            ...
            <div class="min-w-0">
              <div class="font-semibold text-gray-800 dark:text-gray-100 text-sm truncate">{{ s.name }}</div>
              <div class="text-xs text-gray-400 dark:text-gray-500 font-mono truncate" :title="s.path">{{ s.path }}</div>
            </div>
          </div>
          <a-switch
            size="small"
        ...
        <p class="text-xs text-gray-500 dark:text-gray-400 mt-2.5 leading-relaxed line-clamp-3">
    ...
    >
      <p class="text-sm text-gray-500 dark:text-gray-400 mb-4">
        扫描到以下 Agent 已配置的技能，可编辑各 Agent 的配置文件路径后局部「重新扫描」，勾选需要导入的项。导入后会以软链接方式挂载到项目 <code class="text-gray-500 dark:text-gray-400 bg-gray-100 dark:bg-[var(--color-bg)] px-1 rounded">skills/</code> 目录并自动启用（来源更新自动跟随）。
      </p>
        ...
        >
          ...
          <div class="space-y-1.5 pl-1 border-t border-gray-100 dark:border-[var(--color-border)] pt-2.5">
            <div
              ...
              :key="s.dirName"
              class="py-1.5 px-2 rounded-lg hover:bg-gray-50 dark:hover:bg-[var(--color-bg)] transition-colors"
            >
              <div class="flex items-center gap-2.5">
                ...
                <div class="flex-1 min-w-0">
                  ...
                  <div class="text-xs text-gray-400 dark:text-gray-500 font-mono truncate mt-0.5">{{ s.path }}</div>
                </div>
              </div>
              <p v-if="s.description" class="text-xs text-gray-500 dark:text-gray-400 mt-1 ml-7 leading-relaxed line-clamp-2">
                {{ s.description }}
              </p>
            </div>
            <p
