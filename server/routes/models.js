...
import { MODEL_LIST_TYPE, PRESET_VENDOR_LIST_RULES, matchKnownById } from '../lib/modelList.js'

const router = Router()

// 自动获取拿不到时，用内置表/系列规则按模型 id 补全 contextWindow / maxTokens
// （/models 接口通常只返回 id/name，常见模型按 id 精确表或系列前缀兜底）
function withEnriched(models) {
  return (models || []).map((m) => {
    const known = matchKnownById(m && m.id)
    return {
      ...m,
      contextWindow: m.contextWindow || known.contextWindow || undefined,
      maxTokens: m.maxTokens || known.maxTokens || undefined,
    }
  })
}
...
router.post('/models/fetch', async (req, res) => {
  ...
  if (!url) {
    res.json({ models: withEnriched(type.parse({}, rule)) })
    return
  }
  ...
  try {
    ...
    res.json({ models: withEnriched(type.parse(json, rule)) })
  } catch (e) {
